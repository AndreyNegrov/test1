ListeningEnglishContextTraining = function (finishTrainingCallback) {
    Training.call(this, finishTrainingCallback);

    this.trainingName = 'ListeningEnglishContextTraining';

    this.iteration = ListeningEnglishContextTrainingIteration;
...
const ListeningEnglishContextTrainingIteration = function (item, dom, finishIterationCallable) {
    ...
    this.fillDOM = () => {
        ...
        this.context = this.item.additionInfo.examples[0].english.split(' ');

        DOMModifier.setCartPicture(this.item.picture);
        DOMModifier.setMiddleSection(this.item.additionInfo.examples[0].russian);

        this.DOM.highSectionDOM.innerHTML = '';
        this.DOM.footerSectionDOM.innerHTML = '';

        shuffle(this.item.additionInfo.examples[0].english.split(' ')).forEach((word) => {
